...
def path_interpolated_cycle(ctx, cycle):
    segments = graph.cycle_cubic_interpolate(cycle)
...
def path_tangent_cycle(ctx, cycle, smooth=0.3):
    ...
    for i in range(len(cycle)):
        a = cycle[i % len(cycle)].point()
        b = cycle[(i+1) % len(cycle)].point()
        c = cycle[(i+2) % len(cycle)].point()
        d = cycle[(i+3) % len(cycle)].point()
...
def draw_lines(ctx, g):
    ...
    for cycle in graph.node_cycles(g):
        ctx.new_path()
...
def tangent_vector(a, b, c):
    ...
    d_theta = np.arctan2(
        np.dot([0, 0, 1], np.cross(to_a, to_c)),
        np.dot(to_a, to_c))
    if d_theta < 0:
        d_theta += 2*np.pi
